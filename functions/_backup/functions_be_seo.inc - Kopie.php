...
{	global $a1544_mypage;
	...
	*/
    ...
    $cnt .= '<div class="seocu-check">';
		...
		if ($config['be_seo_checks_titledesc']):
			...
			if (!empty($desc)):
				...
				//$cnt .= ($showchecks && count(explode(" ", $desc)) >= $config['be_seo_desc_words']) ? '<li class="'.$css_sub.'"><i class="rex-icon '.$icon_info.'"></i>'.rex_i18n::msg('a1544_seo_desc_words').'</li>' : '';		 	//(alt)
				$cnt .= ($showchecks && a1544_countWords($desc) >= $config['be_seo_desc_words']) ? '<li class="'.$css_sub.'"><i class="rex-icon '.$icon_info.'"></i>'.rex_i18n::msg('a1544_seo_desc_words').'</li>' : '';
			else:
		...
		if ($config['be_seo_checks_header']):
			...
			elseif (empty($h1[0])):
				...
				$cnt .= (count($h1_words) < 2 || mb_strlen($h1cnt) < 20) ? '<li class="'.$css_sub.'"><i class="rex-icon '.$icon_info.'"></i>'.rex_i18n::msg('a1544_seo_h1_short').'</li>' : '';			//Hinweis: H1 ist zu kurz oder nur 1 Wort
			...
			if ($hxcount > 0):
				...
				if (!$hxerror):
					$cnt .= ($showchecks) ? '<li><i class="rex-icon '.$icon_ok.'"></i>'.rex_i18n::msg('a1544_seo_hx_ok').'</li>' : '';
					$cnt .= ($hxcount < 2) ? '<li class="'.$css_sub.'"><i class="rex-icon '.$icon_info.'"></i>'.rex_i18n::msg('a1544_seo_hx_short').'</li>' : '';			//Empfehlung: mehr als 1 Überschrift
					$checks_ok++;
				else:
		...
		if ($config['be_seo_checks_content']):
			...
			if ($berror):
				...
				$cnt .= ($showchecks) ? '<li><i class="rex-icon '.$icon_ok.'"></i>'.rex_i18n::msg('a1544_seo_bolds_ok').'</li>' : '';											//Keine probleme mit bold-Tags gefunden
				$cnt .= (count($bolds) <= 0) ? '<li class="'.$css_sub.'"><i class="rex-icon '.$icon_info.'"></i>'.rex_i18n::msg('a1544_seo_bolds_short').'</li>' : '';			//Empfehlung: bold-Tags nutzen
				$checks_ok++;
			endif;
			$checks++;
		endif;
		...
		if (!empty($keyword)):
			...
			if ($config['be_seo_checks_header']):
				...
				if (count($hx) > 0):
					...
					if ($found >= 2):
						$cnt .= ($showchecks) ? '<li><i class="rex-icon '.$icon_ok.'"></i>'.str_replace("###count###", $found, rex_i18n::msg('a1544_seo_keyhx_ok')).'</li>' : '';
						$checks_ok++;
					else:
			...
			if ($config['be_seo_checks_content']):
				...
				if (count($content_words) > 0):
					...
					if ($found):
						$cnt .= ($showchecks) ? '<li><i class="rex-icon '.$icon_ok.'"></i>'.rex_i18n::msg('a1544_seo_keycnt_atstart_ok').'</li>' : '';
						$checks_ok++;
					else:
			...
			if (count($imgs) > 0 && $config['be_seo_checks_images']):
				...
				//ALT
				if ($found_alt > 0):
					$cnt .= ($showchecks) ? '<li><i class="rex-icon '.$icon_ok.'"></i>'.rex_i18n::msg('a1544_seo_keyimg_alt_ok').'</li>' : '';
					$checks_ok++;
				else:
				...
				//TITLE
				if ($found_title > 0):
					$cnt .= ($showchecks) ? '<li><i class="rex-icon '.$icon_ok.'"></i>'.rex_i18n::msg('a1544_seo_keyimg_title_ok').'</li>' : '';
					$checks_ok++;
				else:
				...
				//SRC
				if ($found_src > 0):
					$cnt .= ($showchecks) ? '<li><i class="rex-icon '.$icon_ok.'"></i>'.rex_i18n::msg('a1544_seo_keyimg_src_ok').'</li>' : '';
					$checks_ok++;
				else:
	...
	//Basis-Analyse zwischenspeichern für DB
	$analysis = $cnt;
	...
	*/
	...
	$wdflink_data = 'data-seocu-aid="'.$actArt.'" data-seocu-cid="'.$actClang.'" data-seocu-aname="'.htmlspecialchars(a1544_removeTags($art->getName())).'"';
		$wdflink = (!$showchecks) ? '<a class="seoculist-morewdf seoculist-morewdf-sidebar" data-toggle="modal" data-target="#seocu-modal" '.$wdflink_data.'>'.$wdflink_name.'</a>' : '<a class="seoculist-morewdf seoculist-morewdf-sidebar" '.$wdflink_data.'>'.$wdflink_name.'</a>';
	$cnt 		= str_replace("###wdflink###", $wdflink, $cnt);
	$analysis	= str_replace("###wdflink###", $wdflink, $analysis);
...
{	global $a1544_mypage;
	...
	$title = aFM_maskChar($title);
		...
		$ptitle = "";
			//$tmp = explode(" ", $title_raw);		 	//(alt)
			$tmp = a1544_countWords($title_raw);
			foreach($tmp as $t):
				$t = trim($t);
				$ptitle .= (mb_strlen($ptitle." ".$t) <= $config['be_seo_title_max']) ? " ".$t : "";
			endforeach;
		$ptitle = trim($ptitle);
		$ptitle .= ($ptitle < $title) ? ' ...' : '';
		*/
...
{	global $a1544_mypage;
	...
	*/
	...
	//Stopwörter
	$stopwords = @$config['be_seo_wdf_stopwords'];
	...
	if (!empty($content) && !empty($a1544_seps) && count($stopwords) > 0):
		//Wörter zählen
		$content .= " #A1544_END_A1544#";
		
		$wc = $wc_wostops = 0;
		$wc_token = array();
		
		for ($word = strtok($content, $a1544_seps); $word != "#A1544_END_A1544#"; $word = strtok($a1544_seps)):
